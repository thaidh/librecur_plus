...
import org.dmfs.rfc5545.calendarmetrics.ChineseCalendarMetrics;
import org.dmfs.rfc5545.calendarmetrics.LunarCalendarUtils;
...
//        if (calendarMetrics instanceof ChineseCalendarMetrics) {
//            int[] lunar = LunarCalendarUtils.convertSolar2Lunar(day1, month, year, 7.0);
//            monthDays = LunarCalendarUtils.getNumDayOfMonth(lunar[2], lunar[1], 1).getNumDay();
//            if (monthDays == 0) {
//                monthDays = LunarCalendarUtils.getNumDayOfMonth(lunar[2], lunar[1], 0).getNumDay();
//            }
//        } else {
//            monthDays = calendarMetrics.getDaysPerPackedMonth(year, month);
//        }

        for (int day : mMonthDays)
        {
            int actualDay = day;
            int[] solar = new int[] {0, 0, 0};
            if (calendarMetrics instanceof ChineseCalendarMetrics) {
                double TZ = TimeZone.getDefault().getOffset(instance) / 3600000;

                solar = new int[]{Instance.dayOfMonth(instance), Instance.month(instance) + 1, Instance.year(instance)};
                int[] lunar = LunarCalendarUtils.convertSolar2Lunar(solar[0], solar[1], solar[2], TZ);
                if (lunar[0] == 0) {
                    lunar = LunarCalendarUtils.convertSolar2Lunar(solar[0], solar[1], solar[2], TZ);
                }
                monthDays = LunarCalendarUtils.getNumDayOfMonth(lunar[2], lunar[1] - 1, 0).getNumDay();
                if (monthDays == 0) {
                    monthDays = LunarCalendarUtils.getNumDayOfMonth(lunar[2], lunar[1] - 1, 1).getNumDay();
                }

                int actualLunarDay = day;
                if (actualLunarDay < 0) {
                    actualLunarDay = actualLunarDay + monthDays + 1;
                } else if (actualLunarDay > 0 && actualLunarDay > 30) {
                   actualLunarDay = -1;
                }


                if ((actualLunarDay > 0 && actualLunarDay < 30) || (actualLunarDay == 30 && monthDays == 30)) {
                    solar = LunarCalendarUtils.convertLunar2Solar(actualLunarDay, lunar[1], lunar[2], lunar[3], TZ);
            ...
            switch (mScope)
            {
                ...
                case MONTHLY:
					...
					if (calendarMetrics instanceof ChineseCalendarMetrics) {
                        if (0 < solar[0])
                        {
                            addInstance(Instance.setYear(Instance.setMonth(Instance.setDayOfMonth(instance, solar[0]), solar[1] - 1), solar[2]));
                        }
                    } else {
                        if (0 < actualDay)
                        {
