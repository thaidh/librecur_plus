...
//     * @param start
    ...
    public long next()
    {
        double TZ = TimeZone.getTimeZone("Asia/Ho_Chi_Minh").getOffset(mNextInstance) / 3600000;
        long result = mNextInstance;
        if (mCalendarMetrics instanceof ChineseCalendarMetrics) {
            int[] solar = new int[] {Instance.dayOfMonth(mNextInstance), Instance.month(mNextInstance) + 1, Instance.year(mNextInstance)};
            int[] lunar = LunarCalendarUtils.convertSolar2Lunar(solar[0], solar[1], solar[2], TZ);
            int numDayOfYear = ChineseCalendarMetrics.getNumDayOfYear(solar[2]);
            int numDayOfCurrentYear = ChineseCalendarMetrics.getNumDayOfYear(solar[2] - 1);
            if (numDayOfYear > numDayOfCurrentYear) {
                numDayOfYear = numDayOfCurrentYear;
            }
            numDayOfYear--;
            int start = LunarCalendarUtils.jdFromDate(solar[0], solar[1], solar[2]);
            solar = LunarCalendarUtils.jdToDate(start + numDayOfYear);
            int[] lunar1 = LunarCalendarUtils.convertSolar2Lunar(solar[0], solar[1], solar[2], TZ);
            ...
            mNextInstance = Instance.setYear(Instance.setMonth(Instance.setDayOfMonth(mNextInstance, solar[0]), solar[1] - 1), solar[2]);
        } else {
